...
function* processGetPage({ name, skip, lastUpdated }) {
    yield put(startRequest());
    yield put(actions.startPageRequest());
    yield put(startCall(requestList, [name, skip ?? 0, lastUpdated]));
    ...
    if (action.type === APP_MESSAGES.AUTHENTICATED_CALL_RESOLVED) {
        ...
        yield put(actions.resolvePageRequest({ name, payload: { items: res.data, new: skip ? false : true } }));
    } else {
...
// function* processGetUpdatedPages({ name, skip, lastUpdated }) {
//     yield put(startRequest());
//     yield put(actions.startPageRequest());
//     yield put(startCall(requestListUpdate, [name, skip ?? 0, lastUpdated]));
//     const action = yield take([APP_MESSAGES.AUTHENTICATED_CALL_RESOLVED, APP_MESSAGES.AUTHENTICATED_CALL_REJECTED]);
//     if (action.type === APP_MESSAGES.AUTHENTICATED_CALL_RESOLVED) {
//         const res = action.payload;
//         yield put(actions.populateState({ name, payload: res.data }));
